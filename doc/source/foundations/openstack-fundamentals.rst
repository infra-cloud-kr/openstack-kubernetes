...
OpenStack 의 주요 서비스는 다음의 표와 같습니다.
...
.. list-table::
   ...
   * - 서비스
     - 역할
   * - **Keystone(키스톤)** — 인증 서비스
     - OpenStack 의 모든 서비스 중앙 인증, 사용자 자격 증명, 역할 기반
   ...
   * - **Swift(스위프트)** — 오브젝트 스토리지 서비스
     - 이미지, 백업 등 대규모 비정형 데이터를 오브젝트 단위로 분산 저장하고
       REST API 로 제공
   * - **Glance(글랜스)** — 이미지 서비스
     - VM 에 사용될 디스크 이미지 및 메타데이터(metadata) 등록, 검색,
       카탈로그화 및 배포 담당
   * - **Horizon(호라이즌)** — 웹 대시보드
...
OpenStack 의 서비스 구조와 컨테이너 배포
========================================

앞에서 소개한 Nova, Neutron, Cinder 와 같은 OpenStack 서비스는 각각
단일 프로세스로 실행되지 않습니다.
...
사용자나 클라이언트가 OpenStack 서비스 API 를 호출하려면 먼저 인증하고
API 엔드포인트(endpoint)를 확인해야 합니다.
Keystone 은 공통 인증을 처리하고 토큰과 서비스 카탈로그를 제공합니다.
사용자나 클라이언트는 카탈로그에 등록된 Nova, Neutron, Cinder 등의
API 엔드포인트를 확인한 뒤 각 서비스의 API 를 직접 호출합니다.

OpenStack 서비스 간 기능 호출도 주로 각 서비스가 제공하는 HTTP API 를
통해 이루어집니다.
같은 서비스에 속한 분산 프로세스 사이에서는 ``oslo.messaging`` 기반
원격 프로시저 호출(remote procedure call, RPC)과 알림을 사용합니다.
리소스 메타데이터와 제어 상태는 일반적으로 SQL 데이터베이스에 저장됩니다.

다음은 이러한 구조를 단순화한 것입니다.
...
이 그림은 일반적인 구조를 단순화한 것입니다.
서비스마다 세부 구성과 통신 방식은 다를 수 있습니다.

컨테이너 배포
-------------

앞에서 살펴본 것처럼 OpenStack 은 역할별 프로세스가 분리되어 있고,
...
운영체제에 설치하고 ``systemd`` 서비스로 관리합니다.
컨테이너 기반 배포에서는 OpenStack 데몬과 데몬 실행에 필요한 사용자 공간
의존성을 컨테이너 이미지로 묶습니다.
운영자는 동일한 이미지를 여러 노드에 배포해 호스트별 패키지 상태에 대한
의존성을 줄이고 일관된 실행 환경을 구성할 수 있습니다.

컨테이너화는 OpenStack 의 서비스 구조를 새로 만드는 것이 아니라 기존의
역할 구분과 통신 구조를 유지하면서 실행 환경을 표준화하는 방법입니다.
이러한 특성 때문에 OpenStack 의 서비스 구조는 컨테이너 기반 배포와 잘 맞습니다.

구성 요소별 배포 고려사항
-------------------------

컨테이너 이미지는 실행 환경을 표준화하지만 구성 요소의 실행 위치,
호스트 접근, 데이터 영속성에 대한 요구사항까지 없애지는 않습니다.
따라서 각 구성 요소의 역할과 의존성에 맞게 배포 방식을 설계해야 합니다.
...
.. list-table::
   ...
   * - 중앙 제어 평면 프로세스
     ...
     - 중앙 제어 노드에 여러 인스턴스로 배치할 수 있습니다.
       데이터베이스, 메시지 브로커 및 다른 서비스의 API 와
       연결되어야 합니다.
   * - 호스트 종속 프로세스
     - ``nova-compute``, Neutron 네트워크 에이전트
     - 자신이 관리하는 하이퍼바이저(hypervisor), Open vSwitch, 네트워크
       인터페이스 등이 있는 노드에서 실행되어야 합니다.
       컨테이너로 실행하더라도 임의의 노드로 이동할 수 없습니다.
   * - 상태 저장 지원 인프라
     - MariaDB, RabbitMQ
...
커널 네트워크 기능에 접근해야 합니다.
Kubernetes 위에 배포하더라도 이러한 노드 종속성은 유지됩니다.

MariaDB 와 RabbitMQ 도 컨테이너로 실행할 수 있습니다.
그러나 컨테이너를 다시 생성하는 것만으로 데이터베이스나 메시지 브로커
클러스터가 복구되는 것은 아니다.
컨테이너 런타임이나 Kubernetes 는 프로세스를 다시 실행할 수 있지만,
데이터 일관성과 클러스터 복구는 각 시스템의 운영 방식에 따라 관리해야 합니다.
...
앞 절에서는 OpenStack 의 서비스 구조가 컨테이너 배포와 잘 맞는 이유를
설명했습니다.
실제 환경에서는 목적과 운영 기반에 따라 서로 다른 배포 방식과 도구를
선택합니다.
여기서는 DevStack, Kolla-Ansible, openstack-helm 의 주요 용도와
실행·관리 기반을 간단히 살펴봅니다.
...
.. list-table::
   ...
   * - Kolla-Ansible
     - Docker 또는 Podman 과 Ansible
     - Kubernetes 없이 컨테이너화된 OpenStack 운영
   * - openstack-helm
     - Kubernetes 와 Helm
     - Helm chart 를 이용한 선언적(declarative) OpenStack 배포
...
DevStack
   개발과 테스트 환경에 OpenStack 을 빠르게 구성하기 위한 스크립트 모음입니다.
   학습과 기능 검증에는 유용하지만 프로덕션 운영을 목적으로 하지 않습니다.

Kolla-Ansible
   Kolla 가 제공하는 OpenStack 서비스 컨테이너 이미지를 Docker 또는 Podman 으로
   실행하고 Ansible playbook 으로 배포·설정합니다.
   Kubernetes 없이 컨테이너화된 OpenStack 을 운영할 때 사용할 수 있습니다.

openstack-helm
   OpenStack 서비스를 Helm chart 로 패키징하여 Kubernetes 위에 배포합니다.
   Kubernetes 의 선언적 리소스 관리와 복구·롤아웃(rollout) 기능을
   OpenStack 운영에 활용합니다.

이 프로젝트는 openstack-helm 을 중심으로 Kubernetes 위에 OpenStack 을 배포하고
운영하는 방법을 다룹니다.
관련 배포 방식과 프로젝트는
:doc:`../openstack-on-kubernetes/approaches` 에서 살펴봅니다.
Kolla-Ansible 과
openstack-helm 의 상세 차이는 :doc:`../openstack-on-kubernetes/comparison` 에서
다룹니다.
...
아래 예시는 리소스를 변경하지 않는 조회 명령입니다.
명령을 실행하기 전에
OpenStackClient 를 설치하고 ``openrc`` 파일이나 ``clouds.yaml`` 파일로
인증 정보를 설정해야 합니다.

서비스 카탈로그를 조회하여 현재 인증 정보와 API 접근이 정상인지 확인합니다.
...
Cinder 와 Swift 가 배포된 환경에서는 블록 볼륨과 오브젝트 스토리지 컨테이너를
조회할 수 있습니다.
...
목록에서 확인한 리소스의 이름이나 ID 를 ``show`` 명령에 전달하면 상세 정보를
확인할 수 있습니다.
...
``<server>``, ``<image>`` 와 같은 값은 실제 리소스의 이름이나 ID 로 바꿔
실행합니다.
